...
chdir( "/ptmp/lscyboz/SHERPA-MC-2.2.2/" + part )
if mode == "grid":
	process = Popen( [ "sbatch"], stdin = PIPE )
        process.stdin.write('#!/bin/sh\n')
        process.stdin.write('#SBATCH --error="slurm.err"\n')
        process.stdin.write('#SBATCH --output="slurm.out"\n')
	process.stdin.write('#SBATCH -J grid\n')
	process.stdin.write('#SBATCH --partition=broadwell\n')
	process.stdin.write('#SBATCH --nodes=4\n')
...
#        process = Popen( ["nohup","mpirun","-n",str(mpithreads),"Sherpa"] + sherpaopts, stdout=outfile )
    ...
    if submit == 'serial':
        ...
        process.stdin.write(' '.join(['srun', 'Sherpa', '-o' ,'0', 'random1:=1$SLURM_ARRAY_TASK_ID'] + sherpaopts)+'\n')
        process.stdin.write('exit 0\n')
        process.stdin.close()
    elif submit == 'condor':
