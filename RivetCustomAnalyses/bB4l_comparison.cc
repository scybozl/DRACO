...
//	_histMlb = bookNLOHisto1D("Mlb", 50, 0, 350);

	_histmWjB = bookNLOHisto1D("mWjB", 40, 150, 200);
	_histmljB = bookNLOHisto1D("mljB", 50, 0, 350);
	_histmjB  = bookNLOHisto1D("mjB", 20, 5, 50);
	_histdeltaR = bookNLOHisto1D("deltaR", 100, 0, 2);
	_histxB   = bookNLOHisto1D("xB", 20, 0, 1);
	_histpTbDec = bookNLOHisto1D("pTbDec", 15, 0, 30);


	_histPt1_T = bookNLOHisto1D("Pt1_T", 25, 0, 400);
        _histEta1_T = bookNLOHisto1D("Eta1_T", 20, -2.5, 2.5);
        _histPhiemu_T = bookNLOHisto1D("Phiemu_T", 20, 0, 2*PI);
        _histdeltaRb_T = bookNLOHisto1D("deltaRb_T", 20, 0, 5);
        _histdeltaRl_T = bookNLOHisto1D("deltaRl_T", 20, 0, 5);
        _histPtMiss_T = bookNLOHisto1D("PtMiss_T", 25, 0, 400);
        _histHT_T = bookNLOHisto1D("HT_T", 20, 0, 1200);
//      _histMlb = bookNLOHisto1D("Mlb", 50, 0, 350);

        _histmWjB_T = bookNLOHisto1D("mWjB_T", 40, 150, 200);
        _histmljB_T = bookNLOHisto1D("mljB_T", 50, 0, 350);
        _histmjB_T  = bookNLOHisto1D("mjB_T", 20, 5, 50);
        _histdeltaR_T = bookNLOHisto1D("deltaR_T", 100, 0, 2);
        _histxB_T   = bookNLOHisto1D("xB_T", 20, 0, 1);
        _histpTbDec_T = bookNLOHisto1D("pTbDec_T", 15, 0, 30);
//	_histmlb = bookNLOHisto1D("mlb", 50, 0, 350);
//	_histmLeptonsjB = bookNLOHisto1D("mLeptonsjB", 50, 0, 350);

	_hist_selected = bookHisto1D("selected", 5, 0, 5);
	_hist_ptlepton = bookNLOHisto1D("ptlepton", 10, 0, 400);
	_nevents = 0;
	_nbjets = 0;
	_nleptons = 0;
...
//      weight *= normWW;
      ...
      foreach(const Jet& j, jets) {
        if (j.absrap() < 2.5 && j.pT() >= 30*GeV) central_jets.push_back(&j);
      }
...
//        }
	...
	if (((*iter)->pdg_id()==-11 || (*iter)->pdg_id()==-13) && Particle(*iter).pT() >= 20*GeV) {leptonsP.push_back(*iter);}
     }
      for (HepMC::GenVertex::particle_iterator iter = WMinus[0]->end_vertex()->particles_begin(HepMC::descendants); iter != WMinus[0]->end_vertex()->particles_end(HepMC::descendants); ++iter) {
        if (((*iter)->pdg_id()==11 || (*iter)->pdg_id()==13) && Particle(*iter).pT() >= 20*GeV) {leptonsM.push_back(*iter);}
     }
...
// TRUTH
     ...
     bool foundM = false, foundP = false, foundb = false, foundbbar = false;
      foreach (const GenParticle* p, Rivet::particles(event.genEvent())) {
	p->print();
        if (foundM == false && p->pdg_id()==13 && Particle(*p).pT() >= 20*GeV) {leptonsM_T.push_back(p); foundM=true;}
	if (foundP == false && p->pdg_id()==-11 && Particle(*p).pT() >= 20*GeV) {leptonsP_T.push_back(p); foundP=true;}
...
if (MET >= 20*GeV) {
	...
	_histPt1->fill(0.5*(b_jets[0]->pT()+b_bar_jets[0]->pT()), event);
	_histEta1->fill(0.5*(b_jets[0]->eta()+b_bar_jets[0]->eta()), event);
	_histdeltaRb->fill(deltaR(b_jets[0]->momentum(), b_bar_jets[0]->momentum()), event);
		//_histPhiemu->fill(deltaPhi(elecFS[0], muonFS[0]), event);
		//_histPhiemu->fill(mapAngle0To2Pi(elecFS[0].momentum().phi() - muonFS[0].momentum().phi()), event);
		//_histdeltaRl->fill(deltaR(elecFS[0], muonFS[0]), event);
	if(!WPlus.empty() && !WMinus.empty()) _histmWjB->fill(((Particle(WPlus[0]).momentum()+b_jets[0]->momentum()).mass() + (Particle(WMinus[0]).momentum()+b_bar_jets[0]->momentum()).mass())/2, event);
	_histmjB->fill((b_jets[0]->momentum().mass() + b_bar_jets[0]->momentum().mass())/2, event);
	if(!leptonsP.empty() && !leptonsM.empty()) {
		 _histmljB->fill(((Particle(leptonsP[0]).momentum()+b_jets[0]->momentum()).mass() + (Particle(leptonsM[0]).momentum()+b_bar_jets[0]->momentum()).mass())/2, event);
}
    ...
    if(b_jets_T.size()>=1 && b_bar_jets_T.size()>=1) {
        _histPt1_T->fill(0.5*(b_jets_T[0]->pT()+b_bar_jets_T[0]->pT()), event);
        _histEta1_T->fill(0.5*(b_jets_T[0]->eta()+b_bar_jets_T[0]->eta()), event);
        _histdeltaRb_T->fill(deltaR(b_jets_T[0]->momentum(), b_bar_jets_T[0]->momentum()), event);
        if (!WPlus.empty() && !WMinus.empty()) _histmWjB_T->fill(((Particle(WPlus[0]).momentum()+b_jets[0]->momentum()).mass() + (Particle(WMinus[0]).momentum()+b_bar_jets[0]->momentum()).mass())/2, event);
        _histmjB_T->fill((b_jets_T[0]->momentum().mass() + b_bar_jets_T[0]->momentum().mass())/2, event);
        if(!leptonsP_T.empty() && !leptonsM_T.empty()) {
                 _histmljB_T->fill(((Particle(leptonsP_T[0]).momentum()+b_jets_T[0]->momentum()).mass() + (Particle(leptonsM_T[0]).momentum()+b_bar_jets_T[0]->momentum()).mass())/2, event);

}


     }
        _histPtMiss_T->fill(MET, event);

	if(!leptonsP_T.empty() && !leptonsM_T.empty()){
        //_histPhiemu_T->fill(deltaPhi(leptonsP_T[0], leptonsM_T[0]), event);
        _histPhiemu_T->fill(mapAngle0To2Pi(Particle(leptonsP_T[0]).momentum().phi() - Particle(leptonsM_T[0]).momentum().phi()), event);
        _histdeltaRl->fill(deltaR(Particle(leptonsP_T[0]), Particle(leptonsM_T[0])), event);
	
	_hist_ptlepton->fill(1/2.0*(Particle(leptonsP_T[0]).pT()+Particle(leptonsM_T[0]).pT()), event);
}
}
...
//		_histMlb->fill(((elecFS[0].momentum() + b_jets_old[1]->momentum()).mass() + (muonFS[0].momentum() + b_jets_old[0]->momentum()).mass())/2, event);
//		}
//		else _histMlb->fill(((elecFS[0].momentum() + b_jets_old[0]->momentum()).mass() + (muonFS[0].momentum() + b_jets_old[1]->momentum()).mass())/2, event);
//	}
//	if(b_jets.size()>=1 && b_bar_jets.size()>=1 && !elecFS.empty() && !muonFS.empty()){
//	  _histmLeptonsjB->fill(((elecFS[0].momentum()+b_jets[0]->momentum()).mass() + (muonFS[0].momentum()+b_bar_jets[0]->momentum()).mass())/2, event);
//	}
//	if(bs.size()>=1 && bbars.size()>=1 && !elecFS.empty() && !muonFS.empty()){
//	   _histmlb->fill(((elecFS[0].momentum()+Particle(bs[0]).momentum()).mass() + (muonFS[0].momentum()+Particle(bbars[0]).momentum()).mass())/2, event);
//	}
...
//	normalize(_histMlb);
	...
	const double norm = crossSection()/sumOfWeights();
        ...
        _histmjB->finalize();
        _histPt1_T->finalize();
        _histEta1_T->finalize();
        _histPhiemu_T->finalize();
        _histdeltaRb_T->finalize();
        _histdeltaRl_T->finalize();
        _histmWjB_T->finalize();
        _histmljB_T->finalize();
        _histmjB_T->finalize();
        _hist_ptlepton->finalize();
...
//	NLOHisto1DPtr _histMlb;
	...
	NLOHisto1DPtr _histPt1_T;
        NLOHisto1DPtr _histEta1_T;
        NLOHisto1DPtr _histPhiemu_T;
        NLOHisto1DPtr _histdeltaRb_T;
        NLOHisto1DPtr _histdeltaRl_T;
        NLOHisto1DPtr _histPtMiss_T;
        NLOHisto1DPtr _histHT_T;
//      NLOHisto1DPtr _histMlb;

//	NLOHisto1DPtr _histmlb;
//        NLOHisto1DPtr _histmLeptonsjB;

	NLOHisto1DPtr _histmWjB;
        NLOHisto1DPtr _histmljB;
        NLOHisto1DPtr _histmjB;
        NLOHisto1DPtr _histdeltaR;
        NLOHisto1DPtr _histxB;
        NLOHisto1DPtr _histpTbDec;

	NLOHisto1DPtr _histmWjB_T;
        NLOHisto1DPtr _histmljB_T;
        NLOHisto1DPtr _histmjB_T;
        NLOHisto1DPtr _histdeltaR_T;
        NLOHisto1DPtr _histxB_T;
        NLOHisto1DPtr _histpTbDec_T;

	Histo1DPtr _hist_selected;
	NLOHisto1DPtr _hist_ptlepton;
	int _nevents;
	int _nbjets;
	int _nleptons;
...
/*
    ...
    NLOHisto1DPtr _histogram(unsigned int histId, unsigned int thresholdId) {
      ...
      if (histId == 0) _hMap.insert(make_pair(hInd,bookNLOHisto1D(1,thresholdId+1,1)));
      else _hMap.insert(make_pair(hInd,bookNLOHisto1D(2,histId,1)));
      return _hMap[hInd];
    }
*/
