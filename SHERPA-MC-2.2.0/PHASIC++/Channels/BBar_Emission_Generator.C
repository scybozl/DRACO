...
(Process_Base *const bviproc,CS_Dipole *const dip)
{
  ...
  std::cout << "PROCESS: " << sproc->Name() << "\n";
  std::cout << "BviProc size: " << bviproc->Size() << "\n";
  std::cout << "SPROC flavours : ";
  for (size_t i=0;i<cfl.size();++i) std::cout << cfl[i].Kfcode() << " ";
  std::cout << "\n";
  for (size_t i(0);i<bviproc->Size();++i) {
    std::cout << "BviProc flavours: ";
    for (size_t f=0;f<(*bviproc)[i]->Flavours().size();++f) std::cout << (*bviproc)[i]->Flavours()[f].Kfcode() << " ";
    std::cout << "\n";
